...
void my_opengl::setUniform(claws::Vect<2, float> const data, char const *target, Program program)
{
  glUniform2f(glGetUniformLocation(program, target), data[0], data[1]);
}

void my_opengl::setUniform(claws::Vect<3, float> const data, char const *target, Program program)
{
  glUniform3f(glGetUniformLocation(program, target), data[0], data[1], data[2]);
}

void my_opengl::setUniform(claws::Vect<4, float> const data, char const *target, Program program)
{
...
Texture my_opengl::loadTexture(std::string const &name)
{
  ...
  char readBuf[4];
  claws::Vect<2u, unsigned int> dim{0u, 0u};
  try {
    file.seekg(10);
